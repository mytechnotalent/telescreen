# Appendix H: Annotated AArch64 Disassembly

Each module's functions, each preceded by its C `@brief`, then its full AArch64 block.
Generated with `clang -O2 -S` on an arm64 host (same ISA as the RP5).
